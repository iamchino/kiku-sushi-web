...
interface MenuPanelProps {
  ...
  description: string;
  ctaLabel: string;
  /** Precio opcional, ej: "\$53.500 por persona" */
  precio?: string;
  ctaHref: string;
  image: string;
  imageAlt: string;
    ...
    >
      <div ref={inViewRef} className={`grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-24 items-center ${isLeft ? "" : "md:[direction:rtl]"}`}>
        ...
        <div className={`max-w-md ${isLeft ? "" : "md:[direction:ltr]"}`}>
          ...
          >
            {title}{" "}
            <span className="font-normal text-v2-champagne">{titleAccent}</span>
          </motion.h2>
          ...
          <motion.a
            ...
            href={ctaHref}
            className="group inline-flex items-center gap-3 text-[13px] uppercase tracking-[0.3em] text-v2-champagne pb-1.5 border-b border-v2-champagne/24 hover:border-v2-champagne transition-all"
          >
            {ctaLabel}
          ...
          {precio && (
            <motion.span
              ...
              className="font-display text-xl text-v2-champagne ml-8 whitespace-nowrap"
            >
              {precio}
            </motion.span>
          )}
